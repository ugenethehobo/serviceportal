  ...
  return (
    <div className={cn('mx-auto w-full max-w-md sm:max-w-xl lg:mx-0 lg:max-w-none', className)}>
      <div className="rounded-xl border border-white/20 bg-white/10 p-1.5 shadow-2xl shadow-black/30 backdrop-blur-md sm:rounded-2xl sm:p-2">
        <div className="relative aspect-[5/3] overflow-hidden rounded-lg bg-slate-950/40 sm:rounded-xl">
          {slides.map((slide, index) => {
            const isActive = index === activeIndex
            const isRasterScreenshot = /\.(png|jpe?g|webp)$/i.test(slide.productImage.src)
            return (
              <div
              ...
              >
                <Image
                  ...
                  priority={index === 0}
                  quality={100}
                  unoptimized={isRasterScreenshot}
                  sizes="(max-width: 640px) 100vw, (max-width: 1024px) 576px, 600px"
