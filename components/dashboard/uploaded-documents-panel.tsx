...
export function UploadedDocumentsPanel({
  ...
  const displayCategories = useMemo(
    () => deriveDocumentCategories(documents),
    [documents]
  )

  const groupedDocuments = useMemo(
    () => groupDocumentsByCategory(documents, displayCategories),
    [documents, displayCategories]
  )

  const categorySuggestions = useMemo(() => displayCategories, [displayCategories])
  ...
  const defaultTitle = scheduleId ? 'Job documents' : 'Uploaded documents'
  const defaultDescription = scheduleId
    ? 'Upload work orders, permits, and other files for this job.'
    : variant === 'portal'
      ? 'Files shared with you by your service provider.'
      : 'Upload contracts, IDs, and other client files separate from estimate PDFs.'
  ...
  const renderDocumentStack = (items: UploadedDocument[]) => {
    if (items.length === 0) {
      return (
  ...
  return (
    <div className="flex flex-col gap-5 flex-1 min-h-0">
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  Type any category name. Empty defaults to {DEFAULT_DOCUMENT_CATEGORY}.
                </p>
              </div>
      ...
      ) : hasCategories && groupedDocuments.length > 0 ? (
