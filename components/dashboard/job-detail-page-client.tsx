...
export function JobDetailPageClient({
  ...
  const jobTabSwitcher = (
    <TabsList className="h-auto w-auto max-w-full flex-wrap justify-center gap-1 overflow-visible max-lg:w-full max-lg:justify-start max-lg:overflow-x-auto max-lg:overflow-y-hidden max-lg:overscroll-x-contain max-lg:touch-pan-x max-lg:flex-nowrap">
      {jobTabs.map((tab) => (
        <TabsTrigger
          key={tab.id}
          value={tab.id}
